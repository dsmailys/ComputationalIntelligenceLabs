...
def TwoCategoricalFeatures(df):
    kf = KFold(n_splits=4)
    i=0
    for train_index, test_index in kf.split(df):
        i=i+1
        print('Fold numer: ', i)
        trainDataSetX = df.iloc[train_index][['PROMOS', 'STYLES']]
        ...
        # plt.plot(testDataSetX[['PROMOS']], prediction, color='blue', linewidth=3)
        # plt.xticks(())
        # plt.yticks(())
        # plt.show()

        # plt.scatter(testDataSetX[['STYLES']], testDataSetY,  color='black')
        # plt.plot(testDataSetX[['STYLES']], prediction, color='blue', linewidth=3)
        # plt.xticks(())
        # plt.yticks(())
        # plt.show()
...
def SixFeatures(df):
    ...
    for train_index, test_index in kf.split(df):
        i=i+1
        print('Fold numer: ', i)
        trainDataSetX = df.iloc[train_index][['PROMOS', 'STYLES', 'DAYS', 'REC', 'CC_CARD', 'WEB']]
...
# 1. using simple Ordinary least squares Linear Regression.
